...
def condition(c):
    ...
    if c.lower() == "brand new":
        factor = 1.0
    elif c.lower() == "like new":
        factor = 0.8
    elif c.lower() == "very good":
        factor = 0.75
    elif c.lower() == "good":
        factor = 0.5
    elif c.lower() == "not specified":
        factor = 0.5
        ...
        print(f"book_pricer: ERROR unknown condition {c}");
        factor = 0.5
...
def estimate(now_year, latent_price, book_data):
    ...
    book_age = age(now_year, book_year) 
       
    if book_age is None:
        return None
   
    price = latent_price*book_format(is_paperback)*book_age*condition(book_condition)
...
def estimate_latent(now_year, data, isbn):
    ...
    if num_points < 3: #add logic when isbn is none
        ...
        for d in data:
            ...
            if not d_isbn is None:
                if not isbn is None and isbn == d_isbn:
                    price = float(d['price'])
                    is_paperback = (d["format"].lower() == "paperback")
                    average_latent_price += price/(book_format(is_paperback)*age(now_year,  d['publish_date'])*condition(d["condition"]))				        
                    count +=1
        if count == 0:
            return None
        else:
    ...
    for d in data:
        ...
        price = float(d['price'])
        is_paperback = (d["format"].lower() == "paperback")
        book_age = age(now_year, d['publish_date']) 
        
        if book_age is None:
            continue
            
        latent = price/(book_format(is_paperback)*book_age*condition(d["condition"]))
