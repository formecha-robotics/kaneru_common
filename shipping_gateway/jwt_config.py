...
AUTH_CALL_MATRIX = {
    ...
    # ---- Frontend checkout (lean quote) ----
    ("POST", "/shipping/domestic"): {
        "callers": ["website_tokyobookshelf"],
        "scopes": ["shipping.domestic"],
    },

    # ---- OMS basket quote (full detail) ----
    ("POST", "/shipping/domestic/basket"): {
        "callers": ["order_gateway"],
        "scopes": ["shipping.domestic.basket"],
    },
...
if ALLOWED_GATEWAY_DETAILS == {} and JWT_PUBLIC_KEY_PATH:
    WEBSITE_PUBLIC_KEY_PEM, ORDER_GATEWAY_PUBLIC_KEY_PEM = split_public_keys(JWT_PUBLIC_KEY_PATH)
    ALLOWED_GATEWAY_DETAILS = {
        "website_tokyobookshelf": WEBSITE_PUBLIC_KEY_PEM,
        "order_gateway":          ORDER_GATEWAY_PUBLIC_KEY_PEM,
    }
