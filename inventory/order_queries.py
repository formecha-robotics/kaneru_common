...
IMAGE_DIR = "/run/inventory_images/"

_SAFE_IDENT_RE = re.compile(r"^[A-Za-z0-9_]+$")

def fetch_inventory_with_desc(company_id: int, inventory_ids: List[int]) -> Dict[int, dict]:
    """
    Runs 2-stage lookup in ONE transaction:
      1) Fetch base inv rows incl cat_short_name + location + condition
      2) For each distinct cat_short_name, query `{prefix}_desc_id` and attach desc data
    Returns a JSON-serializable dict keyed by inv_id.
    """
    ...
    try:
        ...
        q1 = f"""
            SELECT
                ...
                im.depth
            FROM inventory i,
                 inv_location_stock s,
                 inv_category c,
                 inv_location_mapping m,
                 inv_condition_mapping cm,
                 inv_metrics im
            WHERE i.inv_id = s.inv_id
              AND i.inv_cat_id = c.inv_cat_id
              AND m.inv_location_id = s.inv_location_id
              AND cm.inv_condition_id = i.inv_condition_id
              AND im.desc_id = i.inv_desc_id
              AND im.variant_id = i.inv_variant_id
              AND m.company_id = %s
              AND i.inv_id IN ({placeholders})
            ORDER BY i.inv_id ASC;
        """

        cur = conn.cursor(dictionary=True)
        cur.execute(q1, [company_id, *inventory_ids])
        base_rows = cur.fetchall()

        # Prepare output keyed by inv_id
        out: dict[int, dict] = {}
        inv_to_prefix: dict[int, str] = {}
        prefix_to_inv_ids: dict[str, list[int]] = {}

        for r in base_rows:
            inv_id = int(r["inv_id"])
            prefix = r["cat_short_name"]
            inv_desc_id = r["inv_desc_id"]
            inv_desc_id_safe = base64.urlsafe_b64encode(inv_desc_id).decode('utf-8').rstrip("=")
            image_file = inv_desc_id_safe + ".jpg"
            ...
            if os.path.exists(image_file_path):
                try:
                    with open(image_file_path, 'rb') as f:
                        data = f.read()
                        image_bin = base64.b64encode(data).decode("utf-8")
            ...
            out[inv_id] = {
                "inv_id": inv_id,
                "cat_short_name": prefix,
                "location": r["location"],
                "sublocation": r["sublocation"],
                "condition": r["condition"],
                "image" : image_bin,
                "metrics" : {"cat_id" : r["cat_id"], 
            ...
            inv_to_prefix[inv_id] = prefix
            prefix_to_inv_ids.setdefault(prefix, []).append(inv_id)

        # Nothing found -> return empty (still consistent)
        if not out:
            conn.commit()
            return {}

        # ---- Query 2: per-prefix desc lookup ----
        # NOTE: table name is dynamic => must sanitize prefix to avoid SQL injection
        for prefix, inv_ids_for_prefix in prefix_to_inv_ids.items():
            if not _SAFE_IDENT_RE.match(prefix):
                raise ValueError(f"Unsafe cat_short_name for table prefix: {prefix!r}")

            desc_table = f"`{prefix}_desc`"  # per your naming requirement
            desc_title_table = f"`{prefix}_inv_desc_title`"
            desc_desc_table = f"`{prefix}_inv_desc_description`"
            placeholders2 = ",".join(["%s"] * len(inv_ids_for_prefix))
            ...
            q2 = f"""
                ...
                FROM inventory i,
                     ...
                     {desc_desc_table} dd,
                     {desc_title_table} t                     
                WHERE i.inv_desc_id = d.desc_id
                  AND i.inv_id IN ({placeholders2})
                  AND d.inv_desc_id = dd.desc_id
                  AND d.inv_desc_id = t.desc_id
                ORDER BY i.inv_id ASC;
            """

            cur.execute(q2, inv_ids_for_prefix)
            desc_rows = cur.fetchall()

            for dr in desc_rows:
                inv_id = int(dr["inv_id"])
                if inv_id in out:
                    out[inv_id]["description"] = dr["description"]
                    out[inv_id]["title"] = dr["title"]
